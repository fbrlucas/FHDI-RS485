...
class Cmd(object):
    ...
    FS = ('>B','>b','>H','>h','>L','>l','>Q','>q','>f','>d')
    TS = (1,1,2,2,4,4,8,8,4,8)
    ...
    def __init__(self, **kwargs):
        ...
        if 'buffer' in kwargs:
            ...
            if 'type' in self.payload:
                t = self.payload['type']
                if t < 0 and t > POINT_TYPE_DOUBLE:
    ...
    def __decode_val(self,s,t):
        v = unpack(Cmd.FS[t],s)[0]
        return v
        
    def __encode_val(self,v,t):
        s = pack(Cmd.FS[t],v)
        return s
    ...
    def __encode_payload_req(self):
        if self.cmd == SENS_ITF_VER_CMD or \
           ...
           (self.cmd >= SENS_POINT_READ_BASE_CMD and \
            ...
            self.size = 1 + Cmd.TS[self.payload['type']]
            payload_buffer = pack('>B', self.payload['type']) + \
                                self.__encode_val(self.payload['value'],self.payload['type'])
        else:
            logging.error('[%s] Invalid CMD %d' % (get_func_name(), self.cmd))
    ...
    def __decode_payload_req(self, payload):
        ...
        if (self.cmd == SENS_ITF_VER_CMD or self.cmd == SENS_IDENT_CMD) and self.size == 0:
            ...
            if t >= 0 and t <= POINT_TYPE_DOUBLE:
                if self.size == (1 + Cmd.TS[t]): 
                    v = self.__decode_val(payload[1:1 + Cmd.TS[t]],t)
    ...
    def __eq__(self, other):
        ...
        if self.src == other.src and \
           ...
           self.crc == other.crc:
            eq = True
            for k in self.payload.iterkeys():
